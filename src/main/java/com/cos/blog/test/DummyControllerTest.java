...
public class DummyControllerTest {
	...
	/*
	스프링이 RestController라는 어노테이션을 읽어서 DummyControllerTest를 메모리에 띄워줄때
	userRepository 는 null 상태이나 @Autowired를 붙여주면 DummyControllerTest가 메모리에 뜰때
	userRepository도 메모리에 같이 뜬다.
	스프링이 컴포넌트 스캔을 할때 UserRepository안에 있는 UserRepository 인터페이스를 메모리에 띄워주기에
	Autowired를 통해서 메모리에 떠있을 수 있다. 
	 */
	@Autowired //의존성 주입 (DI)
	...
	public User updateUser(@PathVariable int id, @RequestBody User requestUser) { //json 데이터를 요청 => Java Object(MessageConverter의 Jackson라이브러리가 변환해서 받아줌)로 변환해서 받아줌
	...
	*/
	...
	public User detail(@PathVariable int id) {
		...
		// user 4를 찾으면, 내가 데이터베이스에서 못찾아오게되면 user가 null이 될 것이 아니야?
		// 그럼 return null이 리턴이 되므로 프로그램이 문제가 되지 않겠니?
	...
	public String join(User user) {
		...
		System.out.println("createDate:" + user.getCreateDate()); //자바에서 만들어서 insert해줌
